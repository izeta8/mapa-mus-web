...
- **Seguridad y Hardening de Base de Datos (Cuando se implemente Autenticación B2B):**
  ...
  - [ ] **Seguridad en Funciones (RPC):**
    - [x] Limitar la ejecución de la función `advance_winner` (revisar que sea invoker o que valide internamente el `auth.uid()` contra el organizador).
    - [x] Añadir `SET search_path = public` a todas las funciones custom (`advance_winner`, `rollback_winner`, `handle_new_user`, `generate_tournament_short_id`, `update_updated_at_column`) para evitar problemas de búsqueda dinámica.
